...
static float gBgColor[3] = {0.f, 20 / 255.f, 26 / 255.f};                                // color de fondo
...
// Color de líneas (RGB en 0..1)
static float gLineColor[3] = {10 / 255.f, 10 / 255.f, 10 / 255.f}; // valor inicial
// (opcional) grosor de línea
static float gLineWidth = 1.0f;

static void drawLeafRect(const Node *n)
{
    const float x0 = (float)n->x;
    const float y0 = (float)n->y;
    const float x1 = (float)(n->x + n->w);
    const float y1 = (float)(n->y + n->h);
    ...
    if (gDrawLines)
    {
        ...
        // glColor3f(214/255.f, 255/255.f, 172/255.f); // green
        // glColor3f(27/255.f, 100/255.f, 151/255.f);
        // glColor3f(10/255.f, 10/255.f, 10/255.f);

        glColor3f(gLineColor[0], gLineColor[1], gLineColor[2]);
        glLineWidth(gLineWidth); // <-- usa el grosor
        // background: var(--blue500, rgba(38, 139, 210, 1));
...
static void renderQT(const Node *n)
{
    if (!n)
        return;
    if (n->leaf)
...
// ---- Cámara (en coordenadas de imagen: 0..IMG_W x 0..IMG_H)
static float gZoom = 0.6f; // 1 = imagen al 100%
static float gPanX = 0.0f; // desplazamiento del borde izquierdo (px imagen)
static float gPanY = 0.0f; // desplazamiento del borde superior (px imagen)

// util: hacer "fit" a la ventana
static void FitViewToWindow(int fbW, int fbH)
{
    if (IMG_W == 0 || IMG_H == 0)
        return;
    float sx = (float)fbW / (float)IMG_W;
    float sy = (float)fbH / (float)IMG_H;
    gZoom = std::min(sx, sy); // mostrar todo
    // centra la imagen
    float viewW = (float)IMG_W / gZoom;
    float viewH = (float)IMG_H / gZoom;
    gPanX = (-(viewW - IMG_W) * 0.5f); // centrado: deja márgenes iguales
    gPanY = (-(viewH - IMG_H) * 0.5f);
}
...
static void rasterizeQT(const Node *n, int W, int H, std::vector<Color> &out)
{
    ...
    if (n->leaf)
    {
        blitRect(out, W, H, n->x, n->y, n->w, n->h, n->avg);
        return;
    }
    for (int i = 0; i < 4; ++i)
        rasterizeQT(n->ch[i], W, H, out);
}
...
static inline int leafFromIdx(int idx)
{
    idx = std::clamp(idx, 0, 8);
    return 1 << idx;
}
static inline double sdFromIdx(int idx)
{
...
int main(int argc, char **argv)
{
    ...
    while (!glfwWindowShouldClose(win))
    {
        ...
        // --- después de glfwPollEvents();
        int fbW, fbH;
        glfwGetFramebufferSize(win, &fbW, &fbH);

        // Input de cámara (si el mouse no está sobre la UI)
        ImGuiIO &io = ImGui::GetIO();
        if (!io.WantCaptureMouse)
        {
            ...
            {
                // u,v = posición del cursor en [0..1] relativo al viewport
                float mx = io.MousePos.x;
                float my = io.MousePos.y;
                float u = (fbW > 0) ? (mx / (float)fbW) : 0.5f;
                float v = (fbH > 0) ? (my / (float)fbH) : 0.5f;

                // estado actual de vista
                float viewW = (float)IMG_W / gZoom;
                float viewH = (float)IMG_H / gZoom;
                float left = gPanX;
                float top = gPanY;

                // punto del mundo bajo el cursor
                float worldX = left + u * viewW;
                float worldY = top + v * viewH;

                // aplicar factor de zoom
                float factor = std::pow(1.1f, io.MouseWheel); // rueda arriba -> +zoom
                float newZoom = std::clamp(gZoom * factor, 0.05f, 100.0f);

                // actualizar pan para mantener el punto bajo el cursor
                float newViewW = (float)IMG_W / newZoom;
                float newViewH = (float)IMG_H / newZoom;
                gPanX = worldX - u * newViewW;
                gPanY = worldY - v * newViewH;
                gZoom = newZoom;
            }

            // paneo con botón derecho arrastrando
            if (io.MouseDown[1] || io.MouseDown[0])
            { // botón derecho
                float viewW = (float)IMG_W / gZoom;
                float viewH = (float)IMG_H / gZoom;
                // MouseDelta está en píxeles de pantalla; convertir a unidades del mundo
                if (fbW > 0)
                    gPanX -= io.MouseDelta.x * (viewW / (float)fbW);
                if (fbH > 0)
                    gPanY -= io.MouseDelta.y * (viewH / (float)fbH);
            }

            // doble click izquierdo para reset
        ...
        if (ImGui::CollapsingHeader("Image", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            // ImGuiFileDialog::Instance()->Display("PickImage", ImGuiWindowFlags_NoCollapse, ImVec2(400,400));
            // Abrir el diálogo
            if (ImGui::Button("Browse..."))
            {
                ...
                ImGuiFileDialog::Instance()->OpenDialog(
                    "PickImage",
                    "Open image",
                    ".*,.png,.jpg,.jpeg,.bmp,.tga,.gif,.tiff,.webp", // image formats + all
                    config);
                ImGuiFileDialog::Instance()->Display("PickImage", ImGuiWindowFlags_NoCollapse, ImVec2(400, 400));
            }

            // Dibujar/gestionar el diálogo
            if (ImGuiFileDialog::Instance()->Display("PickImage"))
            {
                if (ImGuiFileDialog::Instance()->IsOk())
                {
                    std::string path = ImGuiFileDialog::Instance()->GetFilePathName();
                    gPendingImagePath = path;
                }
            ...
            if (ImGui::Button("Fit"))
            {
                FitViewToWindow(fbW, fbH);
            }
            ImGui::Text("Zoom: %.1f%%", gZoom * 100.0f);
        ...
        if (ImGui::CollapsingHeader("Segmentation", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            ImGui::ColorEdit3("Grid color", gLineColor, ImGuiColorEditFlags_NoInputs); // picker sin inputs numéricos
            ImGui::SliderFloat("Line width", &gLineWidth, 0.1f, 5.0f, "%.3f");
            ImGui::Separator();
            ImGui::Text("Canvas");
            ImGui::ColorEdit3("Background", gBgColor, ImGuiColorEditFlags_NoInputs); // picker
            // Si quieres alpha:
            // ImGui::SliderFloat("Alpha", &gBgAlpha, 0.0f, 1.0f, "%.2f");
        ...
        glClearColor(gBgColor[0], gBgColor[1], gBgColor[2], 1.0f);
        glClear(GL_COLOR_BUFFER_BIT);

        // glClearColor(1,1,1,1);
        // glClear(GL_COLOR_BUFFER_BIT);

        // Configurar proyección ortográfica en espacio de imagen (origen arriba-izquierda)
        glMatrixMode(GL_PROJECTION);
        glLoadIdentity();
        {
            float viewW = (float)IMG_W / gZoom;
            float viewH = (float)IMG_H / gZoom;
            // top-left = (gPanX, gPanY). Y crece hacia abajo: bottom = top + viewH
            glOrtho(gPanX, gPanX + viewW, gPanY + viewH, gPanY, -1.0, 1.0);
        }
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();

        // Dibuja el quadtree en coords de imagen
        renderQT(root);
